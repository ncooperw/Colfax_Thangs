...
//checks for moving car left
function driveWest() {
    previousMap();

    if ($(car).attr("data") === "north") {
        $(car).removeAttr("data", "north").attr("data", "west").attr("src", "assets/images/p1-car-turning-NW.png").removeClass("carUpDown").addClass("carTurning");
        if (pos.top >= 210) {
            car.animate({
                top: "-=20px"
            }, 100);
        ...
        $(car).removeAttr("data", "south").attr("data", "west").attr("src", "assets/images/p1-car-turning-WS.png").removeClass("carUpDown").addClass("carTurning");
        if (pos.top <= 360) {
            car.animate({
                top: "+=20px"
            }, 100);
        ...
        $(car).attr("data", "west").removeClass("carTurning").attr("src", "assets/images/p1-carLeft.png");
        ...
        if (counter === 0 && pos.left < 60) {
            ...
            car.animate({ left: "-=50px"}, 75);
        }
    }
}

//checks for moving car up
function driveNorth() {
    ...
    if ($(car).attr("data") === "east") {
        ...
        if (counter !== 10 || pos.left <= 715) {
            ...
            $(car).removeAttr("data", "east").attr("data", "north");
            car.animate({ top: "-=50px"}, 75);
        }

     } else if ($(car).attr("data") === "west") {
        $(car).removeAttr("data", "west").attr("src", "assets/images/p1-car-turning-NW.png").addClass("carTurning");
        if (counter !== 0 || pos.left > 40) {
            car.animate({
                left: "-=20px"
            }, 100);
            $(car).removeAttr("data", "west").attr("data", "north").addClass("carUpDown");
        }
...
//checks for moving car right
function driveEast() {
    nextMap();

    if ($(car).attr("data") === "north") {
        $(car).removeAttr("data", "north").attr("data", "east").attr("src", "assets/images/p1-car-turning-NE.png").removeClass("carUpDown").addClass("carTurning");
        if (pos.top >= 210) {
            car.animate({
                top: "-=20px"
            }, 100);
        ...
        $(car).removeAttr("data", "south").attr("data", "east").attr("src", "assets/images/p1-car-turning-ES.png").removeClass("carUpDown").addClass("carTurning");
        if (pos.top <= 360) {
            car.animate({
                top: "+=20px"
            }, 100);
        ...
        $(car).attr("data", "east").removeClass("carTurning").attr("src", "assets/images/p1-carRight.png");
        if (counter === 10 && pos.left > 720) {
            responsiveVoice.speak("Look up there...go north!");
        } else if (counter !== 10 || pos.top >= 140 || pos.left <= 690) {
            car.animate({ left: "+=50px"}, 75);
        }
    }
}

//checks for moving car down
function driveSouth() {
    ...
    if (pos.top > 360) {
        ...
        $(car).removeAttr("data", "east").attr("data", "south").addClass("carUpDown");
        if (counter !== 10 || pos.left <= 715) {
            car.animate({
                left: "+=20px"
            }, 100);
        ...
        $(car).removeAttr("data", "west").attr("data", "south").addClass("carUpDown");
        if (counter !== 0 || pos.left >= 40) { 
            car.animate({
                left: "-=20px"
            }, 100);
